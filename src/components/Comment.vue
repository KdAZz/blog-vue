<template>
  <div>
    ...
    <div v-for="(item,i) in comment" :key="i" class="author-title reply-father">
      ...
      <div class="icon-btn">
        <span @click="showReplyInput(i,item.name,item.id)"><i
            class="iconfont el-icon-s-comment"></i>{{ item.childBlogComments.length }}</span>
      ...
      <div class="reply-box">
        <div v-for="(reply,j) in item.childBlogComments" :key="j" class="author-title">
      ...
      <div v-show="_inputShow(i)" class="my-reply my-comment-reply">
        ...
        <div class=" reply-btn-box">
          <el-button class="reply-btn" size="medium" @click="sendCommentReply(item.id)" type="primary">发表评论
          </el-button>
        </div>
      </div>
    </div>
...
<script>
import {addComment, addCount, getBlogComment} from "@/api/blog/blogApi";
...
export default {
  ...
  mounted() {
    this.myHeader = this.avatar
    getBlogComment(this.msg).then((value) => {
      const {data} = value
      this.comment = data.records
    })
  },
